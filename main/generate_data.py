...
def generate(batches_, stages_, sugarMin_, sugarMax_, degMin_, degMax_, concentrate_ = False, ripening_ = 0, inoInf_ = False):
    ...
    if ripening_ == 0:
        ...
        if concentrate_:
            for i in range(batches_):
                const = random.uniform(0.001,(degMax_ - degMin_) / 4.0)
        ...
        if concentrate_:
            for i in range(batches_):
                const = random.uniform(0.001, (ripeningMax - 1.0)/4)
    ...
    if inoInf_:
        ...
        lossMatrix = [[(1.2967+0.1514*(KVector[i]+NaVector[i])+0.2159*NVector[i]+0.9989*IMatrix[i][j]) for j in range(stages_)] for i in range(batches_)]
    else:
        lossMatrix = [[0 for i in range(stages_)] for j in range(batches_)]
    CMatrix = sugarMatrix
    LMatrix = lossMatrix
    SMatrix = [[CMatrix[i][j] - LMatrix[i][j] for j in range(stages_)] for i in range(batches_)]
